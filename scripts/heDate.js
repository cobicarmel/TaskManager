...
function hebYear(year) {
	...
	while (iYear > 0)
	{
		if (iYear>=17 && iYear<=19 && gmtrNum[index]==16)
			index += 2;		//special case for טו and טז
		num = Math.floor(iYear/gmtrNum[index]);
		if (num > 0)
		{
...
function moadim(cday, cmonth, cyear, hday, hmonth, dow, thour){
	if(hmonth == 6) {
		if(hday == 1 || hday == 2)
			return "Rosh Hashana"
